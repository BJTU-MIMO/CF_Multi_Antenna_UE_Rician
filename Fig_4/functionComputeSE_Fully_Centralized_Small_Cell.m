...
for i = 1:nbrOfRealizations
    ...
    if V_d == 1
        ...
        V = ((Hhatallj*F_precoding*F_precoding'*Hhatallj')+C_tot_blk+eyeML)\(Hhatallj*F_precoding);
    end
  ...
  %----Level1
    ...
    for m = 1:M
        ...
        for k = 1:K
            ...
            v = V(:,(k-1)*N+1:k*N); %Extract combining matrix
            numerator = v'*Hallj1(:,(k-1)*N+1:k*N);%sqrt(p(k))*
            denominator = v'*(Hallj1*Hallj1' + C_tot(:,:,m) + eyeL)*v - numerator*numerator';%Dp*
