function [SE] = functionComputeMonteCarloSE_UL_LSFD(H_Combining,HH,HMean,tau_c,tau_p,nbrOfRealizations,N,L,K,M,F_precoding)
...
%Hw = zeros(M*L,nbrOfRealizations,K*N);
SE = zeros(K,1);
...
for i = 1:nbrOfRealizations
    ...
    for m = 1:M
        ...
        for k = 1:K
            ...
            signal_MR((m-1)*N+1:m*N,:,k) = signal_MR((m-1)*N+1:m*N,:,k) + (v'*Hallj(:,(k-1)*N+1:k*N)*F_precoding((k-1)*N+1:k*N,(k-1)*N+1:k*N))/nbrOfRealizations; % (v_mk)'h_mk
    ...
    for k = 1:K
        for l = 1:K
            ...
            Gp_MR_total(:,:,k,l) = Gp_MR_total(:,:,k,l) + (gp_MR(:,:,k,l)*gp_MR(:,:,k,l)')/nbrOfRealizations;%p(l)*
