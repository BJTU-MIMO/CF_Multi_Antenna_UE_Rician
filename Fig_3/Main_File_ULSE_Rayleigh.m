...
p = px*ones(1,K);
 ...
 %Rayleigh
 SE_Monte_MR_Combining_Level4_1 = zeros(K,nbrOfSetups,length(L));
 SE_Monte_MR_Combining_Level4_wo_1 = zeros(K,nbrOfSetups,length(L));
for i = 1:nbrOfSetups
    [channelGain,channelGain_NLoS,channelGain_LoS,UEpositions,APpositions] = RandomAP_generateSetup_Rayleigh_Multi_Antenna_1(M,K,1,1);
for n = 1:length(L)
   tau_p = K*N/w;   
      [HMean_Withoutphase] = RandomAP_generateSetup_Rayleigh_Multi_Antenna_2(channelGain_LoS,UEpositions,APpositions,M,K,L(n),N,1);
        ...
        %Rayleigh
        SE_Monte_MR_Combining_Level4_1(:,i,n) = SE_level4;
        SE_Monte_MR_Combining_Level4_wo_1(:,i,n) = SE_level4_wo;
    end
    disp([num2str(n) ' AP-Antenna out of ' num2str(L)]);
end

save 'Rayleigh.mat' SE_Monte_MR_Combining_Level4_1 SE_Monte_MR_Combining_Level4_wo_1;